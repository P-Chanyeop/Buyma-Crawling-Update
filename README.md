...
### 🔥 최우선 테스트 항목 (v3.4.15+)
- [ ] **과도한 손실 방지**: 가격차이가 최소마진보다 클 때 자동으로 수정 제외 (예: 7,000엔 > 500엔)
- [ ] **정확한 수정 필터링**: 현재가 적정, 손실 예상 상품은 수정 과정에서 자동 건너뛰기
- [ ] **불필요한 다이얼로그 제거**: 가격차이가 0이거나 100엔 미만인 상품의 수정 다이얼로그 완전 차단
- [ ] **통일된 판단 로직**: 가격관리 탭과 주력상품 탭에서 동일한 수정 필요성 판단 기준 적용
- [ ] **UI 상태 복원**: 주력상품 통합처리 완료 후 버튼 상태 정상 복원
- [ ] **가격차이 계산 통일**: 모든 가격분석에서 "현재가 - 최저가" 기준으로 통일
- [ ] **최저가 상품 수정 방지**: 내 상품이 최저가일 때 가격 수정하지 않고 "현재가 적정 (최저가)" 표시
- [ ] **가격분석 속도 향상 확인** (상품명 비교 과정 생략으로 처리 속도 대폭 향상)
- [ ] **최저가 정확성 향상 확인** (상품명 매칭 실패로 인한 최저가 누락 방지)
- [ ] **페이지별 엑셀 자동 내보내기**: 각 페이지 분석-수정 완료 시 자동으로 엑셀 파일에 결과 추가
- [ ] **통합 엑셀 관리**: 하나의 엑셀 파일에 모든 페이지 결과 순차적으로 누적 저장
- [ ] **가격분석 오류 수정**: 문자열-정수 비교 오류 해결 (주력상품 가격분석 시)
- [ ] **re 모듈 import 오류 해결**: 중복 import 제거로 가격분석 안정성 향상
- [ ] **주력상품 엑셀 업로드**: 상품명, 상품ID, 현재가격만으로 일괄 등록 가능
- [ ] **가격분석 진행률 정상화**: 페이지별(100개 단위) 진행률 표시 확인
- [ ] **10개마다 실시간 테이블 업데이트**: 분석 결과를 10개마다 즉시 테이블에 반영 확인
- [ ] **자동 스크롤**: 마지막 분석 완료된 상품으로 테이블 자동 스크롤 확인
...
### v3.4.15 (2025-01-16) - 스마트 가격 수정 로직 완성 & 불필요한 다이얼로그 제거
- **🛡️ 과도한 손실 방지**: 가격차이가 최소마진보다 클 때 자동으로 수정 제외 (예: 7,000엔 > 500엔)
- **🎯 정확한 수정 필터링**: 현재가 적정, 손실 예상 상품은 수정 과정에서 자동 건너뛰기
- **❌ 불필요한 다이얼로그 제거**: 가격차이가 0이거나 100엔 미만인 상품의 수정 다이얼로그 완전 차단
- **⚖️ 통일된 판단 로직**: 가격관리 탭과 주력상품 탭에서 동일한 수정 필요성 판단 기준 적용
- **🔧 UI 상태 복원**: 주력상품 통합처리 완료 후 버튼 상태 정상 복원
- **💡 직관적 상태 표시**: "현재가 적정 (동일가)", "현재가 적정 (최저가)" 등 명확한 상태 구분
...
### v3.4.14 (2025-01-16) - 가격차이 로직 통일 & 최저가 상품 수정 방지 & 엑셀 출력 개선
- **⚖️ 가격차이 계산 통일**: 모든 가격분석에서 "현재가 - 최저가" 기준으로 통일
- **🛡️ 최저가 상품 수정 방지**: 내 상품이 최저가일 때 가격 수정하지 않고 "현재가 적정 (최저가)" 표시
- **📊 주력상품 엑셀 간소화**: 상품명, 상품ID만 입력하면 되도록 현재가격 컬럼 제거
- **📈 엑셀 출력 개선**: 제안가 컬럼 제거, 가격차이를 현재가격-최저가 차이로 명확히 표시
- **🎯 상품명 정제**: 엑셀 출력 시 브랜드와 상품명만 표시 (商品ID 이후 제거)
- **💡 로직 최적화**: 양수(비쌈), 음수(저렴), 0(동일) 직관적 가격차이 표시

### v3.4.13 (2025-01-16) - 페이지별 엑셀 자동 내보내기 & 가격분석 오류 수정
- **📊 페이지별 엑셀 자동 내보내기**: 각 페이지 분석-수정 완료 시 자동으로 엑셀 파일에 결과 추가
- **📁 통합 엑셀 관리**: 하나의 엑셀 파일에 모든 페이지 결과 순차적으로 누적 저장
- **🔧 가격분석 오류 수정**: 문자열-정수 비교 오류 해결 (주력상품 가격분석 시)
- **⚡ re 모듈 import 오류 해결**: 중복 import 제거로 가격분석 안정성 향상
- **📈 실시간 결과 추적**: 페이지별 처리 완료 시마다 엑셀에 즉시 반영
- **💾 데이터 보존**: 가격분석결과_날짜시간.xlsx 형태로 영구 저장

### v3.4.12 (2025-01-16) - 주력상품 엑셀 업로드 & 가격분석 진행률 완전 개선
- **📊 주력상품 엑셀 업로드**: 상품명, 상품ID, 현재가격만으로 일괄 등록 가능
- **⚡ 가격분석 진행률 정상화**: 페이지별(100개 단위) 진행률 표시로 정확한 진행 상황 확인
- **📈 10개마다 실시간 테이블 업데이트**: 분석 결과를 10개마다 즉시 테이블에 반영
- **🔄 자동 스크롤**: 마지막 분석 완료된 상품으로 테이블 자동 스크롤
- **💡 기본값 자동 설정**: 엑셀 업로드 시 나머지 필드는 분석 전 기본값으로 설정
- **🎯 바로 분석 가능**: 엑셀 업로드 후 즉시 가격확인 버튼으로 분석 시작 가능

### v3.4.12 (2025-01-16) - 가격분석 로직 개선 & 속도 향상
- **⚡ 가격분석 로직 개선**: 상품명 검사 없이 1페이지 내 모든 상품 최저가 추출로 정확성 향상
- **🚀 가격분석 속도 향상**: 상품명 비교 과정 생략으로 처리 속도 대폭 향상
- **🎯 최저가 정확성 향상**: 상품명 매칭 실패로 인한 최저가 누락 방지
- **🔧 코드 간소화**: 복잡한 문자열 비교 로직 제거로 안정성 향상
- **📊 처리 효율성**: 1페이지 내 모든 상품 가격을 순차적으로 비교하여 진짜 최저가 발견
- **🛡️ 안정성 강화**: 상품명 추출 실패 시에도 가격 분석 계속 진행
...
86. **불필요한 가격 수정 다이얼로그**: 가격차이가 0이거나 100엔 미만인 상품에서 수정 다이얼로그가 뜨는 문제 해결 (v3.4.15+)
87. **조건문 순서 오류**: price_difference == 0 조건이 price_difference <= 0 조건 뒤에 있어 실행되지 않는 문제 해결 (v3.4.15+)
88. **수정 필요성 판단 불일치**: 가격관리 탭과 주력상품 탭에서 다른 수정 기준 적용 문제 해결 (v3.4.15+)
