...
63. **크롤링 중 프로그램 튕김**: 워커 스레드에서 QApplication.processEvents() 호출로 해결 (v3.4.4+)
64. **크롤링 중 자동 상품명 변경**: itemChanged 시그널 자동 발생 문제 해결 (v3.4.4+)
65. **크롤링 결과 추가 오류**: UI 업데이트 중 예외 발생으로 프로그램 중단 문제 해결 (v3.4.4+)

### 크롤링 안정성 가이드 (v3.4.4+)
- **프로그램 튕김 방지**: 워커 스레드에서 UI 이벤트 처리 완전 제거
- **상품명 변경 제어**: 📝 버튼을 통한 명시적 상품명 변경만 허용
- **예외 처리 강화**: 모든 UI 업데이트 작업에 개별 안전장치 적용
- **테이블 업데이트 안정성**: crawled_products 초기화 및 컬럼별 예외 처리
- **통계 업데이트 안전성**: 통계 함수 실패 시에도 크롤링 계속 진행
- **액션 버튼 개선**: 상품명 변경(📝), 삭제(🗑️) 버튼으로 사용자 제어 향상
